...
def train_epoch(model, opt, scaler, crit, loader, aux_w=0.2):
    ...
    for x, y in tqdm(loader, leave=False):
        ...
        x_fft = rgb_to_fft_amp(x.reshape(-1, *x.shape[2:])).reshape(x.shape)  # [B,T,3,H,W]
...
if __name__ == "__main__":
    ...
    for e in range(1, EPOCHS+1):
        ...
        if auc > best_auc:
            best_auc = auc
            torch.save(model.state_dict(), f"mmhtc_best.pth")
            print("saved mmhtc_best.pth")
